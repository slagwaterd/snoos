...
import { notifyCampaignCompleted } from './notifications.js';
import { applyVariations } from './variations.js';
        ...
        try {
            while (!signal.aborted) {
                ...
                if (currentIndex >= recipients.length) {
                    ...
                    // Send completion notification with CSV
                    ...
                    notifyCampaignCompleted({
                        ...
                        recipients: recipients.map(r => ({
                            ...
                            sentAt: r.sentAt || campaign.updatedAt
                        })),
                        settings
                    }).catch(err => console.error('[Campaign Notify] Error:', err));
                ...
                try {
                    ...
                    const baseSenderName = settings?.senderName || 'IronMail';

                    if (!defaultSender && !campaign.rotateDomains) {
                    ...
                    finalSubject = replaceContactTags(finalSubject);
                    finalBody = replaceContactTags(finalBody);
                    ...
                    await logActivity('VERZENDEN', 'sending', `Verzenden${domainInfo}...`);
