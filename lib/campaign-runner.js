...
class CampaignRunner {
    constructor() {
        ...
        this.version = "3.3.0-TOTAL-CONTROL";
    }
        ...
        try {
            while (!signal.aborted) {
                ...
                if (killSwitch?.active) {
                    console.log(`[Runner] ☢️ GLOBAL KILL SWITCH DETECTED. Terminating loop.`);
                    this.runningLocks.delete(campaignId);
                    return;
                }
                ...
                try {
                    ...
                    // ---- STEP 1: INITIALIZE ----
                    await logActivity('INIT', 'processing', `Analyse voor ${recipient.name}...`);
                    await delay(500);

                    // ---- STEP 2: SUPER-AGGRESSIVE VALIDATION ----
                    ...
                    // Check if it starts with any of these or contains any of these as a standalone part
                    ...
                    // ---- STEP 3: DNS/MX ----
                    await logActivity('DNS', 'checking', `Domein DNS controleren...`);
                    ...
                    if (!dnsOk) {
                        await logActivity('DNS_FAIL', 'blocked', `Geen mailserver voor ${domain}.`);
                        const finalSaveList = await readData('campaigns');
                        const finalIdx = finalSaveList.findIndex(c => c.id === campaignId);
                        if (finalIdx !== -1) {
                            finalSaveList[finalIdx].currentIndex = currentIndex + 1;
                    ...
                    // ---- STEP 4: SMTP VERIFY ----
                    await logActivity('VERIFIEER', 'checking', `Mailbox status verifiëren...`);
                    let boxOk = true;
                    try {
                    ...
                    if (!boxOk) {
                        await logActivity('BOX_FAIL', 'blocked', `Mailbox bestaat niet.`);
                        const finalSaveList = await readData('campaigns');
                        const finalIdx = finalSaveList.findIndex(c => c.id === campaignId);
                        if (finalIdx !== -1) {
                            finalSaveList[finalIdx].currentIndex = currentIndex + 1;
                    ...
                    await logActivity('AI', 'generating', `Mail opstellen in het ${lang}...`);
                    ...
                    // ---- STEP 6: SENDING ----
                    await logActivity('VERZEND', 'sending', `Verzenden via Resend...`);
                    ...
                    // ---- STEP 7: FINALIZE ----
                    await logActivity('KLAAR', 'success', `✅ Verzonden!`);
