    ...
    async runLoop(campaignId, signal) {
        try {
            while (!signal.aborted) {
                ...
                // Helper to log activity to both console AND campaign logs
                const logActivity = async (step, status, message) => {
                    ...
                    if (campaigns[campaignIndex].logs.length > 100) {
                        campaigns[campaignIndex].logs = campaigns[campaignIndex].logs.slice(0, 100);
                    }
                    await writeData('campaigns', campaigns);
                };
                ...
                await logActivity('VALIDATE', emailIsValid ? 'passed' : 'failed', `Score: ${emailScore}${emailIssues.length ? ' - ' + emailIssues.join(', ') : ''}`);

                // SKIP if email doesn't pass validation
                if (!emailIsValid) {
                    await logActivity('BLOCKED', 'skipped', `Email validation failed (score: ${emailScore})`);
                ...
                await logActivity('MX_CHECK', 'checking', `Checking domain: ${emailDomain}`);
                ...
                if (!trustedDomains.includes(emailDomain.toLowerCase())) {
                    try {
                        ...
                        if (!mxRecords || mxRecords.length === 0) {
                            skipDueToMX = true;
                            await logActivity('MX_CHECK', 'failed', `No MX records for ${emailDomain}`);
                        } else {
                            await logActivity('MX_CHECK', 'passed', `MX verified for ${emailDomain}`);
                        }
                    } catch (mxErr) {
                        if (mxErr.code === 'ENOTFOUND' || mxErr.code === 'ENODATA') {
                            skipDueToMX = true;
                            await logActivity('MX_CHECK', 'failed', `Domain invalid: ${emailDomain}`);
                        } else {
                            await logActivity('MX_CHECK', 'warning', `Inconclusive, proceeding anyway`);
                        }
                    }
                } else {
                    await logActivity('MX_CHECK', 'passed', `Trusted domain: ${emailDomain}`);
                }
                ...
                // ═══════════════════════════════════════════════════════════════
                // SMTP MAILBOX VERIFICATION - Check if email actually exists
                // ═══════════════════════════════════════════════════════════════
                await logActivity('SMTP_CHECK', 'checking', `Verifying mailbox exists...`);
                let mailboxExists = true; // Assume true by default

                try {
                    const emailValidator = new EmailValidator();
                    const { validMailbox, reason } = await emailValidator.verify(recipient.email);

                    if (validMailbox === false) {
                        mailboxExists = false;
                        await logActivity('SMTP_CHECK', 'failed', `Mailbox does NOT exist - ${reason || 'rejected'}`);
                    } else if (validMailbox === true) {
                        await logActivity('SMTP_CHECK', 'passed', `Mailbox verified!`);
                    } else {
                        await logActivity('SMTP_CHECK', 'warning', `Inconclusive (catch-all), proceeding...`);
                    }
                } catch (smtpErr) {
                    await logActivity('SMTP_CHECK', 'warning', `Check failed: ${smtpErr.message}, proceeding...`);
                }

                if (!mailboxExists) {
                    await logActivity('BLOCKED', 'skipped', `Mailbox doesn't exist`);
                    campaigns[campaignIndex].currentIndex = currentIndex + 1;
                    await delay(200);
                    continue;
                }
...
Respond with JSON: { "valid": true/false, "issues": ["list of issues if any"] }`;
                                ...
                                if (validation.valid) {
                                    validated = true;
                                    await logActivity('AI_VALIDATE', 'passed', `Email validated on attempt ${attempts}`);
                                } else {
                                    await logActivity('AI_VALIDATE', 'retry', `Validation failed: ${validation.issues?.join(', ')}`);
                                    prompt += `\n\n### PREVIOUS ATTEMPT FAILED VALIDATION\nFix these issues: ${validation.issues?.join(', ') || 'unknown'}`;
                                }
                            } catch (aiErr) {
                                await logActivity('AI_GENERATE', 'error', `AI call failed: ${aiErr.message}`);
                            }
                        }

                        if (!personalized) {
                            await logActivity('BLOCKED', 'skipped', `AI failed to generate email`);
                            campaigns[campaignIndex].currentIndex = currentIndex + 1;
                            continue;
                        }
                    ...
                    await logActivity('SENT', 'success', `✅ Email delivered! ID: ${data.id}`);
                    ...
                    await logActivity('ERROR', 'failed', `Send failed: ${err.message}`);
                    campaigns[campaignIndex].currentIndex = currentIndex + 1;
                    campaigns[campaignIndex].updatedAt = new Date().toISOString();
                }
